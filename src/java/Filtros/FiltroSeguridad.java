...
/**
 *
 * Esta clase se encarga de la seguridad de la pagina web
 * @author Jose Segovia
 */
public class FiltroSeguridad implements Filter {
    ...
    private void doBeforeProcessing(RequestWrapper request, ResponseWrapper response)
        ...
        if (debug) {
            log("SeguridadFiltro:DoBeforeProcessing");
        }
        ...
        String vaccion = ((HttpServletRequest)request).getParameter("vaccion");
        String url = ((HttpServletRequest)request).getRequestURI();
        
        //Si la url que fue llamada contiene movil es inmediatamente despachada, asi como si
        //tambien contiene css
        if(!url.contains("Movil")){
            if(!url.contains("css")){
                
                //Se verifica que la variable e sea null y que la url no contenga vaccion ni /Proyecto1
                //Esto Hace que cualquier intento de ingresar a una pagina sin antes loguearse redireccione a la Login.jsp
                if( e == null){
                    if(!url.contains("vaccion")){
                        if(!url.equals("/Proyecto1/")){
                                response.sendRedirect("/Proyecto1");
                        }
                    }
                }else{
                
                    //Si e no es null esto se encarga de pasar el rol, con sus permisos, del empleado que esta logueado
                    int id_rol = e.getRol();
                    Controladores.ControladorRol cr = new ControladorRol();
                    Rol rol = cr.GetbyId(id_rol);
                    request.setAttribute("isrol", rol);
                    

                    //Esta parte se encarga de que cuando se llama a la primera pagina se redireccione a Principal.jsp en vez de ir a Login.jsp
                    if(url.equals("/Proyecto1/") && vaccion == null ){
    ...
    private void doAfterProcessing(RequestWrapper request, ResponseWrapper response)
            throws IOException, ServletException {
        if (debug) {
            log("SeguridadFiltro:DoAfterProcessing");
        }
    ...
    public void init(FilterConfig filterConfig) {        
        ...
        if (filterConfig != null) {
            if (debug) {                
                log("SeguridadFiltro: Initializing filter");
            }
        }
    }
    ...
    public String toString() {
        if (filterConfig == null) {
            return ("SeguridadFiltro()");
        }
        StringBuffer sb = new StringBuffer("SeguridadFiltro(");
	...
	// You might, for example, wish to add a setParameter() method. To do this
        ...
        public void setParameter(String name, String[] values) {
            if (debug) {
                System.out.println("SeguridadFiltro::setParameter(" + name + "=" + values + ")" + " localParams = " + localParams);
            }
        ...
        public String getParameter(String name) {
            if (debug) {
                System.out.println("SeguridadFiltro::getParameter(" + name + ") localParams = " + localParams);
            }
            if (localParams == null) {
        ...
        public String[] getParameterValues(String name) {
            if (debug) {
                System.out.println("SeguridadFiltro::getParameterValues(" + name + ") localParams = " + localParams);
            }
            if (localParams == null) {
        ...
        public Enumeration getParameterNames() {
            if (debug) {
                System.out.println("SeguridadFiltro::getParameterNames() localParams = " + localParams);
            }
            if (localParams == null) {
        ...
        public Map getParameterMap() {
            if (debug) {
                System.out.println("SeguridadFiltro::getParameterMap() localParams = " + localParams);
            }
            if (localParams == null) {
